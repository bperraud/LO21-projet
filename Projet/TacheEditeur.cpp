...
/*TacheEditeur::TacheEditeur(TacheUnitaire& tacheToEdit, QWidget* parent) : QWidget(parent), tacheU(&tacheToEdit){
    ...
    titreLabel = new QLabel("titre", this);
    titre = new QLineEdit(tacheToEdit.getTitre(), this);
    ...
    descriptionLabel = new QLabel("description", this);
    description = new QTextEdit(tacheToEdit.getDescription(), this);

    dispoLabel = new QLabel("disponibilité", this);
    dispo = new QDateEdit(tacheToEdit.getDateDisponibilite(), this);
    ...
    dureeH = new QSpinBox(this);
        dureeH->setMinimum(0);
        dureeH->setSuffix(" heure(s)");
        dureeH->setValue(tacheToEdit.getDuree().getHeure());
    dureeM = new QSpinBox(this);
        dureeM->setRange(0, 59);
        dureeM->setSuffix(" minute(s)");
        dureeM->setValue(tacheToEdit.getDuree().getMinute());
    ...
    //composite = new QCheckBox(this);
    //    composite->setChecked(!tacheToEdit.isTacheUnitaire());
    //compositeLabel = new QLabel("unitaire", this);
    ...
    HC4 = new QHBoxLayout;
        ...
        HC4->addWidget(predecesseurs);
    HC5 = new QHBoxLayout;
        HC5->addWidget(annuler);
        HC5->addWidget(sauver);
    VC = new QVBoxLayout;
        VC->addLayout(HC1);
        VC->addLayout(HC2);
        VC->addLayout(HC3);
    ...
    QObject::connect(annuler, SIGNAL(clicked()), this, SLOT(close()));
    QObject::connect(sauver, SIGNAL(clicked()), this, SLOT(sauverTacheU()));
}

TacheEditeur::TacheEditeur(TacheComposite& tacheToEdit, QWidget* parent) : QWidget(parent), tacheC(&tacheToEdit) {
    ...
    //composite = new QCheckBox(this);
    //    composite->setChecked(!tacheToEdit.isTacheUnitaire());
    compositeLabel = new QLabel("composite", this);

    composantesLabel = new QLabel("taches composantes", this);
    composantes = new QListWidget(this);
    for (TacheManager::iterator i = TM.begin(); i != TM.end(); ++i){
        QString UneTache = (*i).getTitre();
        QListWidgetItem* item = new QListWidgetItem(UneTache, composantes);
    ...
    QObject::connect(annuler, SIGNAL(clicked()), this, SLOT(close()));
    QObject::connect(sauver, SIGNAL(clicked()), this, SLOT(sauverTacheC()));
}
*/

/*void TacheEditeur::sauverTacheU(){
    TacheManager& TM = *TacheManager::getInstance();
    // Si le titre de la tâche en cours d'édition existe déjà et que ce n'est pas celui de la tâche chargée
    if (TM.isTacheExistante(titre->text())
        && &(TM.getTache(titre->text())) != tacheU){
...
void TacheEditeur::sauverTacheC(){
    TacheManager& TM = *TacheManager::getInstance();
    // Si le titre de la tâche en cours d'édition existe déjà et que ce n'est pas celui de la tâche chargée
    if (TM.isTacheExistante(titre->text())
        && &(TM.getTache(titre->text())) != tacheC){
